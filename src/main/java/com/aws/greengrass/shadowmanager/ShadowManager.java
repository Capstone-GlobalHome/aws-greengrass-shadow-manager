...
public class ShadowManager extends PluginService {
    ...
    protected void install(InstallConfig installConfig) {
        ...
        if (installConfig.configureSyncDirectionConfig) {
            config.lookup(CONFIGURATION_CONFIG_KEY, CONFIGURATION_SYNC_DIRECTION_TOPIC)
                    .dflt(Direction.BIDRECTIONAL)
                    .subscribe((why, newv) -> {
                        Direction newSyncDirection;
                        if (WhatHappened.removed.equals(why)) {
                            newSyncDirection = Direction.BIDRECTIONAL;
                        } else {
                            try {
                        ...
                        if (newSyncDirection.equals(currentDirection)) {
                            logger.atDebug()
                                    .log("Not processing sync direction config change since it is the same %s",
                                            currentDirection);
                            return;
                        }
                        ...
                        if (Direction.FROMDEVICEONLY.equals(newSyncDirection)) {
                            ...
                            if (Direction.FROMCLOUDONLY.equals(currentDirection)) {
                            ...
                            if (Direction.FROMDEVICEONLY.equals(currentDirection)) {
                            ...
                            startSyncingShadows(StartSyncInfo.builder()
                                    .startSyncStrategy(Direction.FROMCLOUDONLY.equals(currentDirection))
                                    .updateCloudSubscriptions(Direction.FROMDEVICEONLY.equals(currentDirection))
                                    .build());
                        }
